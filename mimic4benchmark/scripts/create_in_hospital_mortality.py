    ...
    for patient in tqdm(patients, desc='Iterating over patients in {}'.format(partition)):
        ...
        for ts_filename in patient_ts_files:
            with open(os.path.join(patient_folder, ts_filename)) as tsfile:
                ...
                mortality = int(label_data["Mortality"])
                ...
                # Filter label_data to include only static_vars, keeping the order intact
                filtered_label_data = label_data[static_vars]

                # Extract values and headers from the filtered data
                label_values = filtered_label_data.values.tolist()
                label_header = ','.join(filtered_label_data.index.tolist())

                ts_lines = tsfile.readlines()
                # header = ts_lines[0]
                header = ts_lines[0].strip() + ',' + label_header + '\n'
                ...
                ts_lines = [line.strip() + ',' + ','.join(map(str, label_values)) + '\n' for (line, t) in
